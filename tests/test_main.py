...
from arq import Job, Actor, BaseWorker, concurrent
...
async def test_simple_job_dispatch(tmpworkdir, loop, debug):
    ...
    assert (
        data == ["MockRedisDemoActor", "add_numbers", False, 60, [1, 2], {}, "__id__"]
    )
...
async def test_enqueue_redis_job(actor, redis_conn):
    ...
    # timestamp
    assert 1e12 < data.pop(0) < 3e12
    assert data == ["DemoActor", "add_numbers", False, 60, [1, 2], {}, "__id__"]
...
async def test_dispatch_work(tmpworkdir, loop, caplog, redis_conn):
    ...
    assert (
        caplog.log
        == (
            "MockRedisDemoActor.add_numbers → dft\n"
...
async def test_custom_name(loop, caplog):
    ...
    assert re.match("^<FoobarActor\(foobar\) at 0x[a-f0-9]+>$", str(actor))
...
async def test_call_direct(mock_actor_worker, caplog):
    ...
    log = re.sub("0.0\d\ds", "0.0XXs", caplog.log)
    assert (
        "arq.jobs: dft  queued  0.0XXs → __id__ MockRedisDemoActor.direct_method(1, 2)\n"
        "arq.jobs: dft  ran in  0.0XXs ← __id__ MockRedisDemoActor.direct_method ● 3"
    ) in log
...
async def test_dynamic_worker_custom_queue(tmpworkdir, loop):
    ...
    assert not tmpworkdir.join("add_numbers").exists()
    worker = MockRedisBaseWorker(
        loop=loop, burst=True, queues=["foobar"], shadows=[CustomActor]
    )
    worker.mock_data = actor.mock_data
    await worker.run()
    await actor.close()
