...
def test_settings_changed():
    ...
    assert str(settings) == (
        "<RedisSettings host=localhost port=123 database=0 password=None conn_retries=5 "
        "conn_timeout=1 conn_retry_delay=1 pool_minsize=1 pool_maxsize=10>"
    )
...
def test_timestamp():
    assert (
        7.99 < (datetime.utcnow() - datetime.utcnow()).total_seconds() / 3600 < 8.01
...
async def test_redis_timeout(loop, mocker):
    ...
    with pytest.raises(OSError):
        await r.get_redis()
    assert arq.utils.asyncio.sleep.call_count == 5  # pylint: disable=no-member


async def test_redis_success_log(loop, caplog):
    settings = RedisSettings()
    pool = await create_pool_lenient(settings, loop)
    assert "redis connection successful" not in caplog.text
    pool.close()
    await pool.wait_closed()

    pool = await create_pool_lenient(settings, loop, _retry=1)
    assert "redis connection successful" in caplog.text
    pool.close()
    await pool.wait_closed()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            datetime(2016, 6, 1, 12, 10, 20, microsecond=123_456),
            dict(second=20),
        ),
        (
            datetime(2016, 6, 1, 12, 10, 10),
            datetime(2016, 6, 1, 12, 11, 0, microsecond=123_456),
            dict(minute=11),
        ),
        (
            datetime(2016, 6, 1, 12, 10, 10),
        ...
        (
            ...
            datetime(2032, 2, 28, 0, 0, 0, microsecond=123_456),
            dict(day=28),
        ),
        (
            datetime(2032, 1, 1, 0, 5),
            datetime(2032, 1, 1, 4, 0, microsecond=123_456),
            dict(hour=4),
        ),
        (
            datetime(2032, 1, 1, 0, 0),
            datetime(2032, 1, 1, 4, 2, microsecond=123_456),
        ...
        (
            ...
            datetime(2032, 3, 31, 0, 0, 0, microsecond=123_456),
            dict(day=31),
        ),
        (
        ...
        (
            datetime(2001, 1, 1, 0, 0, 0),
            datetime(2001, 11, 7, 0, 0, 0, microsecond=123_456),
            dict(month=11, weekday=2),
        ),
        (
            datetime(2001, 1, 1, 0, 0, 0),
            datetime(2001, 1, 3, 0, 0, 0, microsecond=123_456),
            dict(weekday="wed"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            3600,
            dict(minute=10, second=20),
            datetime(2016, 6, 1, 12, 10, 20, microsecond=123_456),
        ),
        (68400, dict(hour=3), datetime(2032, 1, 1, 3, 0, 0, microsecond=123_456)),
        (
            68400 * 60,
            dict(day=31, minute=59),
            datetime(2032, 3, 31, 0, 59, 0, microsecond=123_456),
        ),
        (
            68400 * 7,
            dict(weekday="tues", minute=59),
            datetime(2032, 3, 30, 0, 59, 0, microsecond=123_456),
        ),
        (
        ...
        (
            68400 * 365,
            dict(month=10, day=4, hour=23),
            datetime(2032, 10, 4, 23, 0, 0, microsecond=123_456),
        ),
        (
            1,
